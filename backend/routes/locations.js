...
router.post('/', async (req, res) => {
  try {
    ...
    db.run(
      'INSERT INTO locations (warehouse_id, name, code, parent_id, updated_at) VALUES (?, ?, ?, ?, datetime("now"))',
      [wid, name.trim(), code.trim(), Number.isInteger(parent) ? parent : null]
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const db = await getDbAsync();
    const existing = queryOne(db, 'SELECT * FROM locations WHERE id = ?', [id]);
    ...
    db.run('UPDATE locations SET name=?, code=?, parent_id=?, updated_at=datetime("now") WHERE id=?', [n, c, p, id]);
    saveDb();
    const row = queryOne(db, 'SELECT l.*, w.name as warehouse_name FROM locations l JOIN warehouses w ON l.warehouse_id = w.id WHERE l.id = ?', [id]);
    return success(res, row);
  } catch (err) {
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (Number.isNaN(id)) return validationError(res, 'Invalid location id');
    const db = await getDbAsync();
    const existing = queryOne(db, 'SELECT id FROM locations WHERE id = ?', [id]);
    if (!existing) return notFound(res, 'Location');
